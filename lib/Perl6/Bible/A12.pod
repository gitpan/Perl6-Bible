...
[Update: the "C<.>" form of the attribute syntax is now construed to
...
storage you now use "C<!>" in place of the "C<.>".]
...
[Update: it's now I<easier> to write a private attribute than a public
one.  We've change the C<:> twigil to C<!>, and made it optional,
so you can just declare it as C<has $z>, or as C<has $!z> if you wish
to emphasize the privateness of it.]
...
[Update: in reality things are a little looser than that.  If you use
a role as if it were a class, it autoinstantiates an anonymous class
for you that just does that role.  If you use a class as if it were
a role, it takes a role-like snapshot of the current state of the class
and freezes it into an anonymous role.]
...
[Update: that C<.paws> would have to be write C<@.paws> these days unless
you put the invocant into C<$_>.]
...
[Update: there is no C<Class> class now.  The C<Dog> object is of
type C<Dog>, the same type as any of its instances.  It's just not
a real C<Dog>.  It's an abstract C<Dog>.]
...
[Update: bare class declarations now always default to C<our>.  The top
level class ends up in the global space merely because files start out
being parsed in the C<*> package (aka C<GLOBAL::>), so that's where
the first class declaration puts its name.]
...
[Update: every C<Dog> object (class or instance) just points to its
metaclass instance.  The C<Dog> class object isn't either a C<Class>
or a C<MetaClass> object.  Perhaps it does a C<Class> role though.]
...
[Update: the preceding paragraph is really only true if you take it
as talking about the metaclasses, not the classes.  A C<Dog> is not
a C<Module>.  C<Dog.meta> is a metaclass instance that can do modular
and package operations as well as class operations, and all these
...
C<GLOBAL::Fish::Carp> means the same thing.
...
[Update: Private attributes are now marked with C<!> rather than C<:>,
but nowadays you should just use a lexically scoped class if you want
a private class.]
...
[Update: The current lexical class is now named via C<::?CLASS>.
To capture the actual class of the invocant, use a type variable
like C<::RealClass> in the declaration, with or without the C<$self>.]
...
[Update: methods do not set the topic now unless you declare the invocant
with the name C<$_>.]
...
[Update: private methods are marked with C<!> now rather than C<:>.]
...
method call into a list operator by appending a C<:>.]
...
interpreted as arguments now.  However, most of the following is still true.]
...
[Update: the function is now called C<variable()> instead of tied()
...
[Update: we now use C<!> instead of C<.:> and there is no unary form.]
...
[Update: we simply made C<Dog> a C<Dog>-compatible object.]
...
variable is a POD variable.  A private attribute is C<$!foo>.
By the way, lately we've been calling the secondary sigils "twigils".]
...
That binds the class type of the invocant regardless of how instantiated
the invocant is.  Also, the candidate argument need not be supplied.]
...
name to C<CREATE>, just call C<CREATE> first and pass the result of that
as your candidate to C<bless>,]
...
[Update: Sorry, the new C<::T> parametric type syntax doesn't care
...
[Update: the C<$self> here should probably read C<::T>.]
...
by C<CREATE>.  It's just not entirely instantiated until C<BUILDALL> is
done.]
...
[Update: that would be C<< (:$tail, :@legs, *%extraargs) >> now, but
you don't really need the colons since positional declarations
can still be set by name.]
...
[Update: uses C<!> now, no unary form.]
...
[Update: now you can declare a "first multi" that is simulataneously
an ordinary sub and the final arbiter of subsequent multi declarations
in this scope.  This is done with the "C<proto>" keyword in place of
the "C<multi>" keyword".  Oh, and the "C<sub>" is optional with either
"C<multi>" or "C<proto>".]
...
[Update: we now have the "proto" keyword to specifically mark such
a routine.  It also lets the routine supply known argument names for
any multis found in the lexical scope, so the compiler can be smart
about mapping named args to positionals.]
...
[Update: we probably aren't using the "Manhattan" distance indicated above.]
...
which way you write it.  The effect is the same either way.  (Unless
...
[Update: a proto routine can't use "C<next METHOD>" because the original
dispatch list is exhausted.  Maybe.  If so, we can arrange for C<call()>
...
[Update: the intent of the previous paragraph was to describe whether
a role could grant trust on behalf of the composed class, but it
was ambiguously stated, and sometimes misinterpreted.  It seems to me
that a role could grant trust to private routines in its own scope,
however, since they're basically just sub calls anyway.  On the other
hand, the relationship of the role's package namespace to its eventual
lexical namespace is perhaps problematic.  At some point the object
has to keep track of all its cloned role closures and provide the
right one to private sub callers.]
...
[Update: that'd have to be C<self.call()> or C<$.call()> these days.]
...
[Update: that's C<bool::True> these days.]
...
So as you can see, the delegation
...
except for the keyword, which changes to "subset".  And change C<::=> to C<of>.]
...
borrow it from, like "C<use perl5:DBI;>".]
...
[Update: It's not true that the class object is the same as the metaobject.]
...
[Update: There's no C<.dispatcher> since all objects have a C<.meta> now, and
all objects of a class including class objects are of the same type.]
...
[Update: We switched to bare angles for this.  Double angles now
do shell-like interpolation, so double angles relate to single angles
much like double quotes relate to single quotes.]
