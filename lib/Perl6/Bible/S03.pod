...
applied to a parenthesized list, produces an Arglist object instead
of a list of refs.
...
bitterly if it is ever executed.  Actually, it calls "fail".  Variant
C<???> warns, and C<!!!> dies.
...
(or Arglist) into the Arglist being constructed for the current call,
...
as is this:
    ...
    my $args = \(@foo, @bar);	 # construct Arglist object
    push *$args;
