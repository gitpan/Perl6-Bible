...
used as a "kind" when stored in a type variable such as C<^MyClass>.
...
In addition, package literals may be introduced with C<::> when they
...
Within a declaration, the C<^> sigil also declares the visibility of the
type name without the sigil within the scope of the declaration.
The first such declaration within a scope is assumed to be an unbound
type, and takes the actual type of its associated argument.  With
subsequent declarations the use of the sigil is optional, since the
bare type name is also declared.
...
variables are declared somewhere in the dynamic scope with C<env>.
...
C<List> is lazy.  If all of a C<List>'s arguments are fully evaluated
(such as when all the arguments are constants), the C<List> is
promoted to being an immutable C<Tuple>.  (You can use a C<Tuple>
anywhere you can use a C<List>, so in general it doesn't matter.)
...
any existing top-level CPAN package, of course.)
...
lexical variable must be declared using "C<env>" rather than C<my> to be visible via C<CALLER>.
...
To interpolate an unparenthesized class method, use curlies: C<"{Dog.bark}">.
...
package literal marker, or the C<^> type sigil.)  A consequence
of this is that there's no longer any "C<use strict 'subs'>".
