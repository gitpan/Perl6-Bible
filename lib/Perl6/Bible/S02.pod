...
In addition, for constant subscripts, it resolves the symbol at compile time.
(Symbolic refs always delay lookup till run-time.)
...
Any of the short forms that don't begin with C<q> may omit the colon
after an initial "C<q>", so we automatically get the forms:
