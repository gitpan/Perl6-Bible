...
structural type of a variable:
    ...
    &	code
    ::	package/module/class/role/type
...
is subject to:
    ...
    $*foo	global variable
    $?foo	compiler hint variable (already compiled)
    $+foo	compiler hint variable (currently compiling)
    $=foo	pod variable
    $<foo>	match variable, short for $/{'foo'}

Most variables with twigils are implicitly declared, and don't need a
"my" or "our".  Attribute variables are declared with "has", though.
...
pseudo-package C<OUR>.  The C<OUTER> name refers to the MY symbol table
immediately surrounding the current MY, and C<OUTER::OUTER> is the one
...
lexical variable must be declared "C<is env>" to be visible via C<CALLER>.
(C<$_> is always visible. [Conjectural: so are C<$!> and C<$/>.])
If the variable is not visible in the caller, it returns failure.

=item *

[Conjectural] The C<ENV> pseudo-package is just like C<CALLER> except
that it scans outward through all dynamic scopes until it finds a
variable of that name.  If there is no variable of that name, it
looks in the C<*> package.  If there is no variable in the C<*> package,
it looks in C<%*ENV> for the variable name without the sigil, that is,
in the environment variables passed to program.  If the value is
not found there, it returns failure.  (Subprocesses are passed only
the global C<%*ENV> values.  They do not see any lexical variables or
their values.  The C<ENV> package is only for internal overriding of
...
is the variable name, including any sigil.
...
talk about these compiler-dynamic values using the C<$+> sigil/twigil.

References to C<$+> variables are automatically hoisted into the
context currently being compiled.  Setting or temporizing a C<$+>
...
the context is being compiled, an exception is thrown.
    ...
    {
	say $?FOO;		# prints 42
	BEGIN { temp $+FOO = 43 } # temporizes to *compiling* block
	say $?FOO;		# prints 43
	BEGIN { $+FOO = 44 }
	say $?FOO;		# prints 44
	BEGIN { say $+FOO }	# prints 44, but $?FOO probably undefined
    }
    say $?FOO;			# prints 42 (left scope of temp above)
    $?FOO = 45;			# always an error
    $+FOO = 45;			# an error unless we are compiling something

Note that C<< CALLER<$?FOO> >> might discover the same variable as
C<$+FOO>, but only if the compiling context is the immediate caller.
Likewise C<< OUTER<$?FOO> >> might or might not get you to the
right place.  C<$+FOO> goes outwards dynamically until it finds a
compiling scope, and so is guaranteed to find the "right" C<$?FOO>.
...
The currently compiling Perl parser is switched by modifying $+PARSER.
Lexically scoped parser changes should temporize the modification.
Changes from here to end-of-compilation unit can just assign or bind
it.  In general, most parser changes involve deriving a new grammar
and then pointing $+PARSER at that new grammar.  Alternately, the
...
Underscores are allowed between any two digits in a literal number, where
the definition of digit depends on the radix.  Underscores are not allowed
anywhere else in any numeric literal, including next to the radix point
or exponentiator.

=item *

Initial C<0> no longer indicates octal numbers.  You must use an explicit
radix marker for that.  Pre-defined radix prefixes include:
...
to base 36.  (Use 'a' and 'b' for base twelve, not 't' and 'e'.)

Any radix may include a fractional part.  A dot will always be
interpreted as a radix point if possible by the longest-token rule,
so to call a method on a literal with a base greater than 10, the
safe thing is to put a space before the dot:
...
with 'e' or 'E'.  All other radixes must rely on the constant folding
properties of ordinary multiplication and exponentiation.
...
and maybe a trailing Units type for an implied scaling.  Note also that
leading 0 by itself never implies octal in Perl 6.  Also, the hextonum
converter function will interpret leading 0b or 0d as hex digits,
not radix switchers.
...
this is that there's no longer any "C<use strict subs>".
...
pairs.  To align values of option pairs, you may no longer use the
dot postfix forms:
