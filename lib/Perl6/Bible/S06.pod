...
"Trait" is the name for a compile-time (C<is>) property.
...
immediately or passed on as a C<Code> reference argument.


=head2 "Pointy subs"

The arrow operator C<< -> >> is almost a synonym for the anonymous
C<sub> keyword, except that the parameter list of a pointy sub does
not require parentheses, it does not require a preceding comma when
included in a list, and a pointy sub may not be given traits.
...
those bound to optional positional parameters. Arguments destined for named
parameters may come before and/or after the positional parameters,
but all positional parameters must be contiguous in the call syntax.
Named arguments are recognized syntactically at the "comma" level.
Pairs intended as positional arguments rather than named arguments
must be isolated by extra parens:
...
In other words C<:$when> is shorthand for C<:when($when)>.

There is a corresponding shortcut for hash keys, but it replicates the
key name rather than the hash.  So
...
rather than
    ...
    doit :hash{%hash<a>},1,2,3;

Of course, you could always say

    doit *%hash.pairs('a'),1,2,3;

Note that the C<:> prefix is not quite a unary operator.  It just instructs
the following container to return pairs.  If you say
...
which is not what you want.  Instead say

    :%$hashref<b>

[XXX I wonder if we can fudge this the way we fudge "exists" and "delete"?
These are all macros that speak to the container of the following expression
rather than evaluating it completely first.  Unary C<:> could the same,
presumably, in which case it would be perceived as a unary operator,
much like "delete" is.  Then :$hashref<b> could work as expected.  Another
way to look at it is that these are prefix adverbs to the final lookup.]
...
formal parameter is declared with the name C<$_>.  A method's single
...
location or the end of the syntactic list.  XXX This will turn out
useful when we discuss adverbs below.
...
Some functions take multiple Lists that they wish not to be flattened into
one list.  For instance, zip() wants to iterate several lists in parallel,
while array and hash subscripts want to process multidimensional slices.
The set of underlying argument list (List) objects may be bound to a single
array parameter marked with a C<\>:
...
parameters are bound normally up to the List parameter, and then
C<\@args> takes a snapshot of the remaining input without further
attempts at binding.  The only difference from C<\$args> is that
C<\@args> pays attention to semicolons and pipes, while C<\$args>
flattens them.
...
parameter(s) of the subroutine on the sharp end. So both of the calls
...
operation, it must be a variable (or variable declaration), in which
case the list value is bound to the variable. This special case allows
for "pure" processing chains:

    @oddsquares <== map { $_**2 } <== sort <== grep { $_ % 2 } <== @nums;

    @nums ==> grep { $_ % 2 } ==> sort ==> map { $_**2 } ==> @oddsquares;

If multiple pipes terminate in a single slurpy destination, they are processed
sequentially from inside to outside, with C<< <== >> pipes always taking
precedence over C<< ==> >> pipes.  To the slurpy destination it appears to be a
single long list.  (Certain operators, however, may read the list of pipes in
a different order, but only with a specially declared slurpy parameter that
reveals the dimensionality of the list argument.  Multidimensional slices
(specified with semicolons) are treated identically.  For example, the zip()
function uses semicolon to separate its input lists.)
...
The C<^> sigil is short for "type" in much the same way that C<&> is
...
if we don't care about anything but the matching.  Note that if we'd
said

    sub matchedset (::T, T) {...}

it would have been subtly different.  C<::T> declares that there exists
an external type C<T> that we just haven't seen the declaration for
yet.  It doesn't attempt to bind the actual type of of the argument.
It merely specifies that, by the time this function is called, C<T> is
defined and both arguments match that type.
...
notation, though if you want a parameter bound into C<$/>, you have
to say C<< $<eldest> >> within the signature in that case.  Otherwise
it will try to bind an external C<$eldest> instead, and fail if no such
variable is declared.

Note that a rule-embedded signature has no associated "returns"
syntactic slot, so you have to use C<< --> >> within the signature
to specify the return type.

=head2 Attributive parameters

If a submethod's parameter is declared with a C<.> after the sigil (like an
attribute):
...
[Conjecture: within a submethod, attribute names are not virtual if
...
but the dot is ignored for the parameter name, so
...
Fancier type constrains may be expressed through a subtype:
    ...
    subtype Shinola of Any where {.does(DessertWax) and .does(FloorTopping)};
...
associated with.  It declares the new type name in whatever scope
the surrounding declaration is associated lexical scope.
...
The C<^> sigil is just short for 'type', so if your keyboard is
deficient in Latin-1 you can write:

    sub compare (Any type T $x, T $y) {
	return $x eqv $y;
    }
...
can know its context, but outside the sub it just we don't know anything
...
Note that operators "equiv" to relationals are automatically considered
chaining operators.  At other precedence levels, only chaining operators
...
C<{...}> is always a block.  However, if it consists of a single list, the
first element of which is either a hash or a pair, it is executed immediately
to compose a hash reference.
...
scope from which the current block/closure/subroutine was called, provided
that variable is declared "C<env>".  (Implicit lexicals such as C<$_>
are automatically assumed to be environmental.)

C<< ENV::<$varname> >> specifies the C<$varname> visible in the
innermost dynamic scope that declares the variable as "C<env>".
