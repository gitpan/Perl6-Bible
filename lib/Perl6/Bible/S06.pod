...
one list.  For instance, C<zip()> wants to iterate several lists in parallel,
...
array parameter declared with a C<;> twigil:
...
provided you write hte left side as a signature using C<:(...)> notation:
