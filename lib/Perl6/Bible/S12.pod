...
Every object (including any class object) delegates to an instance
of its metaclass.  You can get at the metaclass of any object via
the C<.meta> method.  A class object is just considered an "empty"
instance in Perl 6.  It differs from an instance object not by having
a different type rather in the extent to which it is defined.
Some class objects may tell you that they are defined, while others
may tell you that they are undefined.  That's up to the object, and
depends on how the metaclass chooses to dispatch the .defined method.
...
system, and are in fact called with a different syntax that uses C<!>
in place of the C<.> character.  See below.)
...
However, you can turn extend any of the legal forms above into a list
